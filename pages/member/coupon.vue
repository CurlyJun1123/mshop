    ...
    <mescroll-uni @getData="getCouponData" refs="mescroll" :size="10">
      <block slot="list">
        <block v-if="couponData.length">
          <view class="coupon-item" v-for="(item, index) in couponData" :key="index" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/shop_uniapp/member/coupon_bg.png') + ')' }" @click="isChecked(item)">
            <view class="coupon-title">
              <text class="coupon-money uni-using-hide">
                <text>￥</text>
                {{ parseInt(item.money) }}
              </text>
              <text class="uni-using-hide">{{ $t('member.coupon.min_use_amount', { amount: parseInt(item.at_least) }) }}</text>
            </view>
        ...
        <ns-empty v-else-if="!couponData.length && emptyShow" :text="$t('member.coupon.no_data')"></ns-empty>
      </block>
    </mescroll-uni>
    <view class="footer-wrap" v-if="couponData.length">
      <button type="primary" @click="save()" :disabled="!Boolean(couponArr.length)">{{ $t('member.coupon.send_coupon') }}</button>
    </view>
        ...
        success: (res) => {
          ...
          newArr.forEach((v) => {
            ...
            if (v.validity_type == 0) {
              v.expireTime = this.$t('member.coupon.validity_period') + this.$util.timeStampTurnTime(v.end_time)
            } else if (v.validity_type == 1) {
              v.expireTime = this.$t('member.coupon.valid_days', { days: v.fixed_term })
            }
          })
    ...
    save() {
      ...
      this.$api.sendRequest({
        ...
        success: (res) => {
          ...
          if (res.code == 0) {
            this.$util.showToast({
              title: this.$t('member.coupon.send_success')
            })
            setTimeout(() => {
