<template>
	<view>
		...
		<view class="cret_now" v-if="data.shop_info.is_reopen && (data.shop_info.is_reopen == 2 || data.shop_info.is_reopen == 3)">
			...
			<view class="text">
				<view v-if="data.shop_info.is_reopen == 2">{{ $t('index.shop_expiring') }}</view>
				<view v-else-if="data.shop_info.is_reopen == 3">{{ data.shop_info.shop_status == 0 ? $t('index.shop_paused') : $t('index.shop_expired') }}</view>
				<view class="color-base-text">
					{{ $t('index.shop_remaining_days', { days: data.shop_info.expires_date }) }}
					<text v-if="data.shop_info.is_reopen == 3">{{ $t('index.expired') }}</text>
				</view>
			</view>
			<view class="cret_btn color-base-bg" @click="getProcedureState">{{ data.shop_info.cert_id == 0 ? $t('index.certify_now') : data.is_reopen == 1 ? $t('index.renew_now') : $t('index.renew_now') }}</view>
		</view>
		<view class="shop" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/shop_uniapp/shop_bg.png') + ')' }">
			...
			<view class="shop_base_info" @click="$util.redirectTo('/pages/my/shop/config')">
				...
				<view class="shop_info">
					...
					<view class="shop_other_info">
						<text>{{ $t('index.expiry_date') }}：{{ data.shop_info.expire_time == 0 ? $t('index.permanent') : $util.timeStampTurnTime(data.shop_info.expire_time) }}</text>
					</view>
				</view>
			...
			<view class="trading_statistics margin_none">
				...
				<view class="content">
					...
					<view>
						<view class="color-tip">{{ $t('index.sales_amount') }}（{{ $t('index.currency_unit') }}）</view>
						<view class="num">{{ data[transaction_statistics].order_total }}</view>
					</view>
					<view>
						<view class="color-tip">{{ $t('index.collect_shop') }}</view>
					...
					<uni-grid :column="5" :showBorder="!1">
						...
						<uni-grid-item>
							<view @click="pendingLink('/pages/goods/list', 'status', '4')" class="grid_item">
								...
								<view class="text">{{ $t('index.illegal_removal') }}</view>
							</view>
						</uni-grid-item>
					</uni-grid>
				</view>
					...
					<uni-grid :column="4" :showBorder="!1">
						...
						<uni-grid-item>
							<view @click="$util.redirectTo('/pages/property/settlement/list')" class="grid_item">
								...
								<view class="text">{{ $t('index.shop_settlement') }}</view>
							</view>
						</uni-grid-item>
						<uni-grid-item>
