<template>
  <view class="withdrawal iphone-safe-area">
    ...
    <view class="dl">
      ...
      <view class="dd">
        <block v-if="base_info.status == 0">待审核</block>
        <block v-else-if="base_info.status == 1">待转账</block>
        <block v-else-if="base_info.status == 2">已转账</block>
        <block v-else-if="base_info.status == -1">已拒绝</block>
      </view>
    </view>
    <view class="dl">
